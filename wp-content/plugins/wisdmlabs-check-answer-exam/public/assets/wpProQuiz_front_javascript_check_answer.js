(function($) {
	...
	$.wpProQuizFront = function(element, options) {
		...
		*/
		...
		/**
		 * Wisdmlabs Tweak : Added unlockResponse parameter for unlocking the response.
		 * Called only from checkQuestion method.  
		 * 
		 */
		var readResponses = function(name, data, $question, $questionList, lockResponse,unlockResponse) {
			//if (config.ld_script_debug == true) {
			...
			if(unlockResponse == undefined) unlockResponse = false;
			...
			var func = {
				singleMulti: function() {
					...
					if(unlockResponse){
						$questionList.find('.wpProQuiz_questionInput').prop("disabled", false);
					}
				...
				matrix_sort_answer: function() {
					...
					if(unlockResponse){
						$question.find('.wpProQuiz_sortStringList, .wpProQuiz_maxtrixSortCriterion').sortable("enable");
					}
				},
				...
				free_answer: function() {
					...
					if(unlockResponse){
						$li.find('.wpProQuiz_questionInput').prop('disabled', false);
					}
					response = value;
				},
				...
				essay: function() {
					...
					if(unlockResponse){
						$questionList.find('.wpProQuiz_questionEssay').attr('disabled', false);
			...
			marker: function(e, correct) {
				if(!bitOptions.disabledAnswerMark) {
					if(correct) {
						...
						e.removeClass('wpProQuiz_answerIncorrect');
					} else {
...
//				$e.find('.wpProQuiz_cloze input').val('');
			...
			/**
			 * Wisdmlabs : Function called for clikcing CheckAnswer Button
			 * This functions locks the answer first and
			 * then send ajax to check if question is correct or incorrect
			 *
			 * @param  {[type]} endCheck [description]
			...
			checkQuestion: function(list, endCheck) {
				...
				//wisdmlabs : This is the ajax called via Check Answer Button 
				...
				plugin.methode.ajax({
					...
					list.each(function() {
						...
						if ( typeof json[question_id] !== 'undefined' ) {
							...
							//Marker - Only show incorrect if incorrect and
							//correct if answer is correct, not both
							plugin.methode.markCorrectIncorrect(result, $this, $questionList);
							
							//here we will do things to unlock and lock all answers
							if(result.c) {
								...
								/**
								 * WisdmLabs show check untill it is right, unlock answer to question 
								 */
								$this.find(globalNames.check).show();
								$this.find(globalNames.skip).hide();
								$this.find(globalNames.next).hide();
								responses[question_id] = readResponses(name, data, $this, $questionList, false,true);					
							}
			...
			markCorrectIncorrect: function(result, $question, $questionList) {
				...
				$questionIndex = $($question).attr('data-pos');
				...
				switch(result.e.type) {
					...
					case 'sort_answer':
						...
						$items.each(function(i, v) {
							var $this = $(this);
							if(result.c && (result.e.c[i] == $this.attr('data-pos'))){
...
//						$questionList.sortable("destroy");
			...
			initQuiz: function() {
				...
				/**
				 * WidsmLabs : Check Answer button Clicked
				 */
