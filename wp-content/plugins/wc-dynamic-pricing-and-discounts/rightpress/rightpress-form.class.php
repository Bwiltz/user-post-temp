...
global $rightpress_form_version;

if (!$rightpress_form_version || $rightpress_form_version < $version) {
    $rightpress_form_version = $version;
}

/**
 * Proxy Class
 */
if (!class_exists('RightPress_Form')) {

final class RightPress_Form
{
    ...
    public static function __callStatic($method_name, $arguments)
    {
        ...
        // Get main class name
        $class_name = 'RightPress_Form_' . $rightpress_form_version;
 ...
 * Main Class
 */
if (!class_exists('RightPress_Form_21')) {

final class RightPress_Form_21
{
    ...
    private static function attributes($params, $custom = array(), $type = 'text')
    {
        ...
        foreach ($attributes as $attribute) {
            if (isset($params[$attribute]) && !RightPress_Helper::is_empty($params[$attribute])) {
        ...
        foreach ($params as $param_key => $param) {
            if (RightPress_Helper::string_begins_with_substring($param_key, 'data-') && !in_array($param_key, $attributes, true)) {
    ...
    private static function label_html($params)
    {
        ...
        if (!empty($params['id']) && isset($params['label']) && !RightPress_Helper::is_empty($params['label'])) {
